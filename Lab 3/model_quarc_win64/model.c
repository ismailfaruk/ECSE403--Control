...
void model_output(int_T tid)
{
   ...
   *  Sin: '<Root>/Sine Wave'
   */
  u = (sin(model_P.SineWave_Freq * model_M->Timing.t[0] + model_P.SineWave_Phase)
       * model_P.SineWave_Amp + model_P.SineWave_Bias) + 0.0;

  /* Saturate: '<Root>/Saturation' */
  model_B.Saturation = u >= model_P.Saturation_UpperSat ?
    model_P.Saturation_UpperSat : u <= model_P.Saturation_LowerSat ?
    model_P.Saturation_LowerSat : u;
  if (rtmIsMajorTimeStep(model_M)) {
    ...
    /* S-Function Block: model/HIL Write Analog (hil_write_analog_block) */
    {
      ...
      result = hil_write_analog(model_DWork.HILInitialize_Card,
        &model_P.HILWriteAnalog_Channels, 1, &model_B.Saturation);
      if (result < 0) {
        msg_get_error_messageA(NULL, result, _rt_error_message, sizeof
          (_rt_error_message));
        rtmSetErrorStatus(model_M, _rt_error_message);
  ...
  /* Derivative: '<Root>/Derivative1' */
  {
    ...
    real_T timeStampA = model_DWork.Derivative1_RWORK.TimeStampA;
    real_T timeStampB = model_DWork.Derivative1_RWORK.TimeStampB;
    real_T *lastU = &model_DWork.Derivative1_RWORK.LastUAtTimeA;
    if (timeStampA >= t && timeStampB >= t) {
      model_B.Derivative1 = 0.0;
    } else {
      real_T deltaT;
      ...
      if (timeStampA < timeStampB) {
        if (timeStampB < t) {
          lastTime = timeStampB;
          lastU = &model_DWork.Derivative1_RWORK.LastUAtTimeB;
        }
      ...
      deltaT = t - lastTime;
      model_B.Derivative1 = (model_B.positionGain - *lastU++) / deltaT;
    }
  }
...
void model_update(int_T tid)
{
  /* Update for Derivative: '<Root>/Derivative1' */
  {
    real_T timeStampA = model_DWork.Derivative1_RWORK.TimeStampA;
    real_T timeStampB = model_DWork.Derivative1_RWORK.TimeStampB;
    real_T* lastTime = &model_DWork.Derivative1_RWORK.TimeStampA;
    real_T* lastU = &model_DWork.Derivative1_RWORK.LastUAtTimeA;
    if (timeStampA != rtInf) {
      if (timeStampB == rtInf) {
        lastTime = &model_DWork.Derivative1_RWORK.TimeStampB;
        lastU = &model_DWork.Derivative1_RWORK.LastUAtTimeB;
...
void model_derivatives(void)
{
  ...
  {
    ((StateDerivatives_model *) model_M->ModelData.derivs)->TransferFcn_CSTATE =
      model_B.Derivative1;
    ((StateDerivatives_model *) model_M->ModelData.derivs)->TransferFcn_CSTATE +=
      (model_P.TransferFcn_A)*model_X.TransferFcn_CSTATE;
  }
}
...
void model_initialize(boolean_T firstTime)
{
  ...
  /* external mode info */
  model_M->Sizes.checksums[0] = (542600865U);
  model_M->Sizes.checksums[1] = (1923316608U);
  model_M->Sizes.checksums[2] = (1103018394U);
  model_M->Sizes.checksums[3] = (3230226340U);
...
void MdlInitializeSizes(void)
{
  ...
  model_M->Sizes.numBlocks = (15);     /* Number of blocks */
  model_M->Sizes.numBlockIO = (5);     /* Number of block outputs */
  model_M->Sizes.numBlockPrms = (87);  /* Sum of parameter "widths" */
}
...
void MdlInitialize(void)
{
  ...
  model_DWork.Derivative1_RWORK.TimeStampA = rtInf;
  model_DWork.Derivative1_RWORK.TimeStampB = rtInf;
}
