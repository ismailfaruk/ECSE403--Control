...
typedef RT_MODEL_Project_Model_Correct RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Project_Model_Correct(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Project_Model_Correct_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(Project_Model_Correct_DWork.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Project_Model_Correct -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (Project_Model_Correct_DWork.HILReadEncoderTimebase_Task, (t_clock)
         Project_Model_Correct_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
