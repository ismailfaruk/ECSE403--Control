...
typedef RT_MODEL_model_sim RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * model_sim(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(model_sim_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(model_sim_DWork.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://model_sim:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: model_sim -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(model_sim_DWork.HILReadEncoderTimebase_Task,
        (t_clock) model_sim_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
